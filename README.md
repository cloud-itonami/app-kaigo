...
問い合わせ導線を 1 ページに載せた SPA と、それを配信する
Cloudflare Worker（XRPC を 3 本公開）から成る。
**SPA は 2026-09-08 に SvelteKit から shadow-cljs + reagent + kotoba-ui
（murakumo-studio 構成）へ移行した。** UI ソースは repo 直下の `src/cloud_itonami/app_kaigo/`
+ `web/`、成果物 `web/dist/` は追跡対象（rare-earth と同じ運用）。
...
| パス | 中身 | 手元で動くか |
|---|---|---|
| **`src/cloud_itonami/app_kaigo/`（state/ui/desktop.cljs）+ `web/`** | shadow-cljs + reagent + kotoba-ui の 1 ページ SPA（2026-09-08 に SvelteKit から移行）。ロボット 3 機種タブ・住宅改修試算・支給限度額表・6 ステップの申請フロー・料金 3 プラン・FAQ。旧 `appview/kaigo-hp/svelte/` は撤去済み | **動く**（[quickstart](docs/operator-quickstart.md)。compile exit 0 + headless Chrome でタブ切替・住宅改修試算・モバイル nav まで実測） |
...
コピーして追跡）は静的に `<title>` と meta description を持つ。**残るは
`SoftwareApplication` の JSON-LD だけで、これは `desktop.cljs/set-head!` が
実行後に注入する**（旧 `<svelte:head>` と同じ挙動を移植した）。

**prerender はまだ無い。** 旧 SvelteKit では `prerender = true` を足して
実際に測ると `Error: 404 /signup (linked from /)` で build が止まった ——
クローラが次の §3 を報告するためである。`/signup` を作るか CTA を
張り替えてからでないと入れられない。

### 3. `/signup` が無い —— 主要 CTA 5 本すべての行き先

`ui.cljs` は `/signup` を **5 箇所**で参照する（デスクトップナビ「無料で始める」・
モバイルナビ・ヒーロー CTA・料金 Basic「無料で試す」・料金 Robot Pro
「14 日間無料トライアル」＝`/signup?plan=pro`）。**signup に相当する view は
どこにも定義されていない（SPA は単一 view のみ）。**
...
| # | 止まる場所 | 実測されるエラー | 直せるか |
|---|---|---|---|
| 1 | （旧 `assets.directory` = `./svelte/build`） | 移行で `./web/dist` に張り替え済み。dev ブートストラップが追跡されているので SPA build 無しで解決する | **解消済み** |
...
**(a) 支給限度額の早見表が 2 段ずれている。** `state.cljs` の `care-limits` と `src/app.ts` の
...
`care_level` 1〜5 に割り当てており、UI（`ui.cljs` の limits 表）はそれを
...
**(b) 自己負担割合を要介護度から導いている。** `state.cljs` の `benefit-calc` と `src/app.ts` の
`calcHousingReformBenefit` はともに「要介護度 3 以上なら 1 割、それ以外 2 割」、UI は
...
**このページは自分で自分を否定している** —— 同じ画面の早見表の脚注に
「2・3 割負担の方は所得に応じて異なります」と正しく書いてある。

なお住宅改修費の**支給限度基準額 200,000 円**と、要介護認定の 6 ステップ、
...
1. **§5 の 2 箇所を直す。** 公開画面に出る給付額の誤りで、影響は利用者に直接届く。
   表は `state.cljs` と `src/app.ts` に**二重に持たれている**ので、片方だけ直すと
   ずれ続ける —— 先に 1 箇所へ寄せる。
